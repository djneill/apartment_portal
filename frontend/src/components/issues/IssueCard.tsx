...
const IssueCard: React.FC<IssueCardProps> = ({
  ...
  return (
    <article className="relative p-4 bg-white rounded-3xl shadow-[0_4px_8px_rgba(0,0,0,0.15)] w-[298px] max-sm:w-full">
      ...
      <button
        className={`flex absolute justify-center items-center w-10 h-10 bg-white border border-black border-solid right-[15px] rounded-[100px] top-[15px] ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
        onClick={onClick}
        disabled={disabled}
      >
